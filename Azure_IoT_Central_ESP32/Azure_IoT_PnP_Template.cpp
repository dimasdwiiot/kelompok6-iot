...
#define TELEMETRY_PROP_NAME_Illuminance "Illuminance"
...
static float read_LDR() {
  int analogValue = analogRead(20);  // Membaca nilai dari pin LDR
  float light = analogValue; 
  return light;  // Mengembalikan nilai tegangan LDR
}

static float read_HCSR04() {
  ...
  if (duration == 0) {
        return 400;  // Jika timeout, kembalikan jarak besar (misalnya 400 cm)
    }
    ...
    size_t* payload_buffer_length)
{
  ...
  az_span json_span;
  float Illuminance, Distance;

  // Membaca data dari sensor LDR dan HC-SR04
  Illuminance = read_LDR();
  ...
  // Menambahkan nilai LDR ke payload JSON
  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR("Illuminance"));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding LDR property name to telemetry payload.");
  rc = az_json_writer_append_double(&jw, Illuminance, DOUBLE_DECIMAL_PLACE_DIGITS);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding LDR value to telemetry payload.");

  // Menambahkan nilai HC-SR04 ke payload JSON
  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR("Distance"));
  ...
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding Distance value to telemetry payload.");
